...
extension Car{
    func runEngine(){
        action(action: .engineOn)
    }
    func stopEngine(){
        action(action: .engineOff)
    }
}
...
extension SportCar: CustomStringConvertible{
    var description: String {
        return "mark: \(carMark) max_speed: \(maxSpeed) current_speed: \(currSpeed) is_engine_run:\(isEngineRun)"
    }
}
...
extension TrunkCar: CustomStringConvertible{
    var description: String {
        return "mark: \(carMark) max_volume: \(maxBagVolume) current_bag_volume: \(bagVolume) is_engine_run:\(isEngineRun)"
    }
}
